...
class WaypointFollower():
    def __init__(self):
        ...
        self.publish_to_arduino_steer = rospy.Publisher('/auto_cmd/steer', Int16, queue_size=15)
        self.publish_to_arduino_throttle = rospy.Publisher('/auto_cmd/throttle', Int16, queue_size=15)

        self.publish_to_arduino_auto = rospy.Publisher('/auto_mode', Bool, queue_size=15)
    ...
    def steer_control(self, error_y, error_yaw):
        ...
        #dt term could be indlcued in the constant term 
        self.error_y_sum += error_y
        self.error_yaw_sum += error_yaw

        steer = kp_y*error_y + kp_yaw*error_yaw + ki_y*(self.error_y_sum) + ki_yaw*self.error_yaw_sum
        #self.error_acc = steer

      ##########################  ##implementing stanley steering controller#############################
        k_e = 0.5
        # the position error term is added  comment out the following line to remove the stanley

        #steer = steer + math.atan2( self.ego_vx ,k_e*error_y )# this one has better performance
        steer = kp_yaw*error_yaw + ki_yaw*self.error_yaw_sum  + math.atan2( self.ego_vx ,k_e*error_y )
#######################################################################################################
        # Control limit
    ...
    def publish_command(self, steer, accel):
        ...
        #reversed
        steer_msg = Int16(np.clip(1430,1300,1600))
        throttle_msg = Int16(np.clip(-1*(steer / np.deg2rad(17.75))*200+ 1500,1300,1700))
        
        msg_true = Bool(True)
        self.publish_to_arduino_auto.publish(msg_true)
        rospy.loginfo("steer = "+str(steer_msg)+" throtle = "+ str(throttle_msg)+"\n")
        self.publish_to_arduino_steer.publish(steer_msg)
        self.publish_to_arduino_throttle.publish(throttle_msg)
...
def main():
    ...
    WPT_CSV_PATH = rospack.get_path('waypoint_follower') + "/wpt_data/real_path.csv"
    csv_data = pd.read_csv(WPT_CSV_PATH, sep=',', header=None)
    wpts_x = csv_data.values[:,0]-2
